...
import {
  DashboardService,
  DashboardStats,
  RecentTradeDto as RecentTradeServiceDto,
  WeeklyReportDto as WeeklyReportServiceDto,
  ComparativeAnalysisDto as ComparativeAnalysisServiceDto,
...
export class DashboardController {
  ...
  @ApiResponse({
    status: 200,
    description:
      'Dashboard statistics (followers, positions, trades by status, copy/fail rates)',
  ...
  @Get('recent-trades')
  @ApiOperation({
    summary: 'Recent trades (optionally only copied / my trades)',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
    type: Number,
    description: 'Max number of trades (1–100, default 20)',
  })
  @ApiQuery({
    name: 'onlyCopied',
    required: false,
    type: Boolean,
    description: 'If true, return only COPIED trades (my copy executions)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Number of weeks to return (1–52, default 12)',
  })
  @ApiResponse({
    status: 200,
    description: 'Weekly aggregated reports with per-wallet breakdown',
  ...
  @ApiOperation({
    summary: 'Comparative analysis: bot vs copied traders (leaders)',
  })
  @ApiResponse({
    status: 200,
    description: 'Bot aggregate and per-leader copy/skip/fail stats',
