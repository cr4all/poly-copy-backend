...
export class DashboardStatsDto {
  ...
  @ApiProperty({
    description: 'Number of active (enabled) followers',
    example: 4,
  })
  ...
  @ApiProperty({
    description: 'Trades successfully copied (all time)',
    example: 42,
  })
  tradesCopied: number;
  ...
  @ApiProperty({
    description:
      'Trades copied in the last 7 days (closer to portfolio activity)',
    example: 8,
  })
  ...
  @ApiProperty({
    description: 'Total latency in ms of the last successfully copied trade',
    required: false,
  })
  lastCopyLatencyMs?: number | null;

  @ApiProperty({
    description: 'Average total copy latency in ms (copied trades only)',
    required: false,
  })
  avgCopyLatencyMs?: number | null;

  @ApiProperty({
    description: 'Last fetch latency (leader trade → we saw it) in ms',
    required: false,
  })
  lastFetchLatencyMs?: number | null;

  @ApiProperty({
    description: 'Last execution latency (we saw it → order sent) in ms',
    required: false,
  })
  ...
  @ApiProperty({
    description: 'Average execution latency in ms',
    required: false,
  })
